# Cryptography Implementations Combined (Version 2)
...
import secrets
import math
from typing import Tuple, NamedTuple, List, Union # Ensure NamedTuple is imported here
...
# --- Helper Functions (GCD, LCM, ModInverse) ---
...
def egcd(a: int, b: int) -> Tuple[int, int, int]:
    ...
    """
    x0, x1, y0, y1 = 1, 0, 0, 1
    # Save original a and b for the final check if needed, though not strictly necessary for return value
    # orig_a, orig_b = a, b 
    while b != 0:
        q, a, b = a // b, b, a % b
        x0, x1 = x1, x0 - q * x1
        y0, y1 = y1, y0 - q * y1
    # gcd is a, coefficients are x0, y0
    # assert orig_a * x0 + orig_b * y0 == a # Optional check: ax + by = gcd
    return (a, x0, y0)
...
class ElGamalParams(NamedTuple):
    ...
    g: int  # Generator
    q: int  # Order of the generator g (prime)
...
def generate_elgamal_params(key_size: int = 2048) -> ElGamalParams:
    """Generates parameters (p, q, g) for the ElGamal signature scheme.
    Ensures p is a safe prime (p = 2q + 1, where q is prime).

    Args:
        key_size: The desired bit length for the prime p. Recommended >= 2048.

    Returns:
        An ElGamalParams object containing the prime p, prime q, and generator g.
    ...
    while True:
        # Generate q first, then check if p = 2q + 1 is prime
        q = number.getPrime(key_size - 1, randfunc=os.urandom)
        p = 2 * q + 1
        if number.isPrime(p):
            break
        # print(f"p={p} (derived from q={q}) is not prime, regenerating q...") # Reduce verbosity
    print(f"Generated safe prime p (bit length {p.bit_length()}) with prime q = (p-1)/2")

    # Find a generator g of the subgroup of order q.
    # Any element h where h^q mod p = 1 and h != 1 is a generator.
    # A common method is to find h such that h^2 mod p != 1, then g = h^2 mod p.
    while True:
        h = number.getRandomRange(2, p - 1, randfunc=os.urandom)
        g = pow(h, 2, p)
        if g != 1 and pow(g, q, p) == 1: # Check if g has order q
            break
    print(f"Found generator g of order q") # Reduce verbosity
    return ElGamalParams(p=p, g=g, q=q)
...
def sign_message_elgamal(private_key: ElGamalPrivateKey, message: bytes) -> ElGamalSignature:
    ...
    if not message:
        raise ValueError("Message cannot be empty.")
    p, g, q = private_key.params.p, private_key.params.g, private_key.params.q
    x = private_key.x
    h_obj = SHA256.new(message)
    h = int.from_bytes(h_obj.digest(), byteorder='big') # Corrected escaping

    while True:
        k = number.getRandomRange(1, q, randfunc=os.urandom) # 1 <= k < q
        r = pow(g, k, p)
        if r == 0: # Should not happen if g is chosen correctly, but check anyway
            continue
        try:
            k_inv = modInverse(k, q)
        except ValueError: # k has no inverse mod q (should not happen if q is prime and 1 <= k < q)
            continue

        # Calculate s = (h - x*r) * k^-1 mod q
        term1 = h % q
        term2 = (x * r) % q
        s_unsigned = (term1 - term2 + q) % q # Ensure result is non-negative before multiplying
        s = (s_unsigned * k_inv) % q

        if s != 0: # Ensure s is not 0
            break
...
def verify_signature_elgamal(public_key: ElGamalPublicKey, message: bytes, signature: ElGamalSignature) -> bool:
    ...
    if not message:
        raise ValueError("Message cannot be empty.")
    p, g, q = public_key.params.p, public_key.params.g, public_key.params.q
    y = public_key.y
    r, s = signature.r, signature.s

    # Check if r and s are in the valid range
    if not (0 < r < p and 0 < s < q):
        # print(f"Signature validation failed: r or s out of range (r={r}, s={s}, p={p}, q={q})") # Reduce verbosity
        return False
    ...
    try:
        # Calculate left side: g^h mod p
        v1 = pow(g, h, p)

        # Calculate right side: (y^r * r^s) mod p
        term_yr = pow(y, r, p)
        term_rs = pow(r, s, p)
        v2 = (term_yr * term_rs) % p

        is_valid = (v1 == v2)
        # if not is_valid: # Reduce verbosity
        #      print("Signature validation failed: Verification equation g^h = y^r * r^s (mod p) does not hold.")
...
def run_elgamal_example():
    ...
    try:
        ...
        # print(f"Parameters: p={params.p}, q={params.q}, g={params.g}") # Reduce verbosity
        print("\nGenerating ElGamal key pair...")
        private_key, public_key = generate_elgamal_keys(params)
        print("Private Key x: <hidden>")
        print(f"Public Key y: ...{str(public_key.y)[-20:]}") # Reduce verbosity
        message = b"This is the message to be signed using ElGamal."
        print(f"\nOriginal Message: {message.decode()}")
        print("Signing the message...")
        signature = sign_message_elgamal(private_key, message)
        print(f"Generated Signature: r=...{str(signature.r)[-20:]}, s=...{str(signature.s)[-20:]}") # Reduce verbosity
        print("\nVerifying the signature with the correct public key...")
...
def sign_message_dsa(private_key: DsaKey, message: bytes) -> bytes:
    ...
    if not message:
        raise ValueError("Message cannot be empty.")
    h = SHA256.new(message)
    # Use 'fips-186-3' which is compatible with FIPS 186-4 for PyCryptodome
    signer = DSS.new(private_key, 'fips-186-3') # Corrected escaping
    signature = signer.sign(h)
    return signature
...
def verify_signature_dsa(public_key: DsaKey, message: bytes, signature: bytes) -> bool:
    ...
    # Check signature length based on q
    q_bytes = (public_key.q.bit_length() + 7) // 8
    if len(signature) != 2 * q_bytes:
         # print(f"Warning: Signature length ({len(signature)}) does not match expected ({2 * q_bytes}). Verification might fail.") # Reduce verbosity
         pass # Allow verification attempt anyway

    h = SHA256.new(message)
    verifier = DSS.new(public_key, 'fips-186-3') # Corrected escaping
    try:
        verifier.verify(h, signature)
        return True
    except ValueError:
...
def run_dsa_example():
    ...
    try:
        ...
        # Check if attributes exist (they should for PyCryptodome keys)
...
def generate_sss_shares(secret: int, k: int, n: int, p: int = _SSS_PRIME) -> List[Tuple[int, int]]:
    ...
    if n >= p:
        # While mathematically possible, using x=1..n requires n < p.
        # If n >= p, we'd need to choose distinct x values differently.
        raise ValueError(f"Number of shares N ({n}) must be smaller than the prime modulus ({p}) for this implementation.")

    # Generate random coefficients for the polynomial P(x) of degree k-1
    # P(x) = secret + a1*x + a2*x^2 + ... + a(k-1)*x^(k-1)
    coeffs = [secret]
    for _ in range(k - 1):
        coeff = secrets.randbelow(p) # Generate random coefficient in [0, p-1]
        coeffs.append(coeff)

    # Generate shares (x_i, y_i) where y_i = P(x_i) mod p
    shares = []
    for i in range(1, n + 1):
        x_i = i # Use simple x coordinates 1, 2, ..., n
...
def reconstruct_sss_secret(shares: List[Tuple[int, int]], p: int = _SSS_PRIME) -> int:
    ...
    # Lagrange Interpolation to find the constant term P(0)
    secret = 0
    for j in range(k):
        x_j, y_j = shares[j]
        ...
        for m in range(k):
            if m != j:
                x_m = shares[m][0]
                numerator = (numerator * (-x_m + p)) % p # Use -x_m mod p for numerator
                diff = (x_j - x_m + p) % p # Ensure positive difference for denominator
                if diff == 0:
                     # This should not happen if x_coords are unique
                     raise ValueError(f"Internal error: division by zero in Lagrange (x_{j}={x_j}, x_{m}={x_m})")
        ...
        try:
            ...
            raise ValueError(f"Could not compute modular inverse for denominator {denominator} mod {p}. Check shares or prime.")

        # l_j(0) = product( (-x_m) / (x_j - x_m) ) for m != j
        lagrange_basis_at_zero = (numerator * inv_denominator) % p

        # Add term y_j * l_j(0) to the sum
...
def run_sss_example():
    ...
    try:
        ...
        secret_int = int.from_bytes(secret_message.encode("utf-8"), "big") # Corrected escaping
        ...
        print(f"Prime Modulus (P): ...{str(prime)[-20:]} (bit length: {prime.bit_length()})") # Reduce verbosity

        if secret_int >= prime:
            print("Error: Chosen prime is too small for the secret integer.")
            print("Please increase the prime size (e.g., 512 bits) or use a shorter secret.")
            return
        if n >= prime:
            print("Error: Number of shares N must be less than the prime P.")
            return
        ...
        for i, share in enumerate(shares):
            print(f"  Share {i+1}: ({share[0]}, ...{str(share[1])[-20:]})" ) # Reduce verbosity
        ...
        try:
            # This reconstruction should yield an incorrect result, not necessarily raise an error
            ...
            # It's possible, though unlikely, that the incorrect reconstruction matches the secret
            if reconstructed_fail == secret_int:
                print("Warning: Reconstruction with insufficient shares unexpectedly matched the secret!")
            else:
                print("Reconstruction with insufficient shares yielded incorrect secret as expected.")
        except ValueError as e:
            # This might happen if k=2 and only 1 share is provided
...
# --- Helper Functions (gcd, lcm, modInverse already defined) ---
...
class PaillierPrivateKey(NamedTuple):
    ...
    # Optionally store p and q for CRT optimization (not implemented here)
    # p: int
    # q: int
...
def generate_paillier_keys(key_size: int = 2048) -> Tuple[PaillierPublicKey, PaillierPrivateKey]:
    ...
    Raises:
        ValueError: If key_size is too small.
        RuntimeError: If modular inverse mu cannot be computed.
    """
    if key_size < 1024:
        raise ValueError("Key size should be at least 1024 bits, 2048 recommended.")

    print(f"Generating two primes for {key_size}-bit modulus...")
    # Ensure p and q are distinct primes of roughly equal size
    ...
    while True:
        ...
        if p != q and gcd(p * q, (p - 1) * (q - 1)) == 1:
            # Ensure gcd(n, phi(n)) = 1 is often desired, though not strictly required by base Paillier
            # More importantly, ensure p != q
            break
        # print(f"Regenerating q (p={p}, q={q})...") # Reduce verbosity
    ...
    # Select generator g. A common choice is g = n + 1.
    # Check if g is suitable (order is a multiple of n)
    g = n + 1
    # Verify that mu exists. L(g^lambda mod n^2) must have an inverse mod n.
    ...
    try:
        ...
        # This should theoretically not happen if p, q are distinct primes and g=n+1
        raise RuntimeError("Could not compute modular inverse mu. Check prime generation or g selection.")
...
def encrypt_paillier(public_key: PaillierPublicKey, plaintext: int) -> int:
    ...
    # Choose random r such that 0 < r < n and gcd(r, n) = 1
    while True:
...
def homomorphic_add_paillier(public_key: PaillierPublicKey, c1: int, c2: int) -> int:
    ...
    Dec(c1 * c2) = Dec(c1) + Dec(c2) mod n.
    """
    n_sq = public_key.n_sq
    # Ensure ciphertexts are valid before operating
    if not (0 <= c1 < n_sq and 0 <= c2 < n_sq):
        raise ValueError("Invalid ciphertexts for homomorphic addition.")
    c_sum = (c1 * c2) % n_sq
    return c_sum
...
def homomorphic_multiply_const_paillier(public_key: PaillierPublicKey, c: int, k: int) -> int:
    ...
    Dec(c^k) = k * Dec(c) mod n.
    """
    n, n_sq = public_key.n, public_key.n_sq
    # Ensure ciphertext is valid
    if not (0 <= c < n_sq):
        raise ValueError("Invalid ciphertext for homomorphic multiplication.")
    # k can be any integer, but the result is effectively k mod n
    # Handle negative k if necessary (using modular inverse for c)
    if k < 0:
        # Compute c^-1 mod n_sq
        try:
            c_inv = modInverse(c, n_sq)
            k_abs = abs(k)
            c_prod = pow(c_inv, k_abs, n_sq)
        except ValueError:
            raise ValueError(f"Cannot compute inverse of ciphertext {c} mod {n_sq} for negative constant.")
    else:
        c_prod = pow(c, k, n_sq)
    return c_prod
...
def run_paillier_example():
    ...
    try:
        key_size = 1024 # Use 1024 for example speed, 2048+ recommended for security
        ...
        print(f"Negative Constant (k_neg): {k_neg}")
        ...
        # --- Homomorphic Multiplication by Constant k --- 
        ...
        expected_prod = (m1 * k) % public_key.n # Multiplication by k is modulo n
        ...
        # --- Homomorphic Multiplication by Negative Constant k_neg --- 
        print(f"\nPerforming homomorphic multiplication (c2^k_neg mod n^2) with k_neg={k_neg}...")
        c_prod_neg = homomorphic_multiply_const_paillier(public_key, c2, k_neg)
        print(f"Resulting Ciphertext (c_prod_neg): ...{str(c_prod_neg)[-20:]}")
        decrypted_prod_neg = decrypt_paillier(private_key, public_key, c_prod_neg)
        # Expected result is (m2 * k_neg) mod n. Python's % handles negative numbers correctly.
        expected_prod_neg = (m2 * k_neg) % public_key.n
        print(f"Decrypted Product: {decrypted_prod_neg}")
        print(f"Expected Product (m2 * k_neg mod n): {expected_prod_neg}")
        assert decrypted_prod_neg == expected_prod_neg

        print("\nPaillier example completed successfully.")
        print("Note: Paillier is partially homomorphic (supports addition and multiplication by constant).")
...
def main():
    ...
    print("=============================================")
    print(" Cryptography Implementations Demonstration (V2) ")
    print("=============================================")
    ...
    # Use standard apostrophe, should be safe now.
